...
| Contact form | ✅ verified | **Email-first** (commit `6906ac2`): submit → emails Omnia via FormSubmit → shows **"Message sent ✓"** in the UI. WhatsApp is a secondary "or message on WhatsApp" button carrying the same pre-filled text. Tested live 2026-09-02 — FormSubmit returns `success:true`, confirmation renders, form resets. |
...
| 2026-09-02 | **email-first form verified** | — | — | — | — | Commit `6906ac2`. Real Chrome: filled + clicked "Send message" → **"Message sent ✓" rendered**, form reset, FormSubmit `OPTIONS` 200 + POST `{"success":"true"}`. Direct endpoint re-check also `success:true`. **Two test emails sent to Omniaamer835@gmail.com** ("Omnia Self-Test" + "confirm #2") — check inbox / spam to confirm receipt from her side. |
